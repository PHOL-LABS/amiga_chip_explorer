...
function WaveformPlaceholder({ direction }: { direction: PinData["direction"] }) {
  ...
  const getColor = () => {
    switch (direction) {
      case "PWR": return "#ff6b6b";
      case "GND": return "#6b9bff";
      case "IN": return "#7ECF7E";
      case "OUT": return "#E8A000";
      case "BI": return "#60a5fa";
      default: return "#666";
    }
  };
  ...
  return (
    <div className="relative rounded overflow-hidden" style={{ background: "#0a0a0a", border: "1px solid #1e1e1e" }}>
      {/* Grid */}
      <svg width="100%" height="80" viewBox="0 0 200 80" preserveAspectRatio="none">
        {/* Grid lines */}
        {[0, 1, 2, 3].map(i => (
          <line key={`h${i}`} x1="0" y1={20 * (i + 1)} x2="200" y2={20 * (i + 1)} stroke="#111" strokeWidth="0.5" />
        ))}
        {[0, 1, 2, 3, 4, 5, 6, 7].map(i => (
          <line key={`v${i}`} x1={25 * (i + 1)} y1="0" x2={25 * (i + 1)} y2="80" stroke="#111" strokeWidth="0.5" />
        ))}
        {/* Glow effect */}
        <defs>
          <filter id="waveGlow">
        ...
        />
        {/* Trigger marker */}
        <line x1="20" y1="0" x2="20" y2="80" stroke="#333" strokeWidth="0.5" strokeDasharray="2,2" />
        <polygon points="20,0 24,4 16,4" fill="#333" />
      </svg>

      {/* GIF placeholder overlay */}
      <div
        className="absolute inset-0 flex flex-col items-center justify-center gap-1 gif-placeholder"
        style={{ background: "rgba(0,0,0,0.7)" }}
      >
        <Icon name="PhotoIcon" size={20} style={{ color: "#333" }} />
        <span className="font-mono text-[9px] tracking-widest" style={{ color: "#333" }}>
          GIF PLACEHOLDER
        </span>
        <span className="font-mono text-[9px]" style={{ color: "#222" }}>
          Replace with oscilloscope GIF
        </span>
      </div>
    </div>
  );
...
export default function SignalViewer({ pin, chipName, partNumber, onClose }: SignalViewerProps) {
  if (!pin) {
    return (
      <div
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
        ...
        <div>
          ...
          <WaveformPlaceholder direction={pin.direction} />
        </div>
        ...
        >
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="font-mono text-[10px]" style={{ color: "#444" }}>NOTES</span>
          </div>
          <p className="font-mono text-[10px] leading-relaxed" style={{ color: "#333" }}>
            GIF placeholders above should be replaced with actual oscilloscope captures
            or logic analyzer recordings of the {pin.name} signal during normal Amiga operation.
            Measurements taken at IC pin level with 10× probe.
          </p>
        </div>
      </div>
    </div>
